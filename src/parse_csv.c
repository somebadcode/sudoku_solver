...
static const char *csv_error_string[] = {FOREACH_CSVERROR(GENERATE_STRING)};

const char *get_csv_error(csv_error err) { return csv_error_string[err]; }
	...
	*position   = 0;
	...
	for (int y = 0; y < 9; y++) {
		for (int x = 0; x < 9; x++) {
			...
			// Advance position up to the end of the token so that
	...
	// Return an error if there's any trailing data since we don't want to
	// let the user assume that the input is correct. This error can be
